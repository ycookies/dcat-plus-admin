...
> 目标：使用 Laravel Blade 独立实现角色创建/编辑页，一次完成路由权限和菜单绑定；不使用 Dcat Form，同时保持现有表结构、旧权限和多应用行为兼容。
>
...
- 不修改现有 RBAC 数据表结构。
- 不删除、不覆盖手工创建的权限。
- 路由扫描只辅助创建新权限，角色已绑定且无法映射到当前路由的旧权限由服务端静默保留。
...
- 角色编辑界面只保留“资源路由”和“单路由”；旧权限、通配符权限和逻辑权限不再作为普通可选项暴露。
- 保存时自动合并角色原有的未映射权限，不依赖前端隐藏字段，既避免篡改也防止历史权限丢失。
...
- PHP 8.3 语法检查：通过。
- Blade 全量缓存编译：通过。
- 单元测试：路由目录、权限解析、历史权限保留、菜单树安全化、内部 Token 和失败关闭等共 11 个测试、58 个断言通过。
...
- 旧的宽泛权限不再向角色操作者展示，但已有角色绑定会在保存时自动保留，不会被路由权限同步覆盖。
