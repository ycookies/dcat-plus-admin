...
class ScaffoldController extends Controller {
    ...
    protected function removeRoutesFromFiles($route_path, $is_add_admin_api, $is_add_member_api)
    {
        ...
        if ($is_add_admin_api) {
            $request = request();
            $api_route_path = $request->input('api_route_path');
        ...
        if ($is_add_member_api) {
            $request = request();
            $member_api_route_path = $request->input('member_api_route_path');
    ...
    public function appendMenu() {
        ...
        if ($is_add_menu && !empty($menu_name) && !empty($route_path)) {
            ...
            $parent_menu_id = $request->input('parent_menu',0);
    ...
    public function appendPermissions() {
        ...
        if (empty($route_path) || empty($menu_name)) {
            return false;
        }
        ...
        // PostgreSQL 在手动插入固定ID后，序列可能未同步，先自愈一次。
        ...
        $permissionsinfo =  $permissionsModel::updateOrCreate(
        ...
        if ($role) {
            // 将权限附加到角色（通过中间表关联）
            $role->permissions()->syncWithoutDetaching([$permissionsinfo->id]);
        }
    }

    /**
     * PostgreSQL下同步模型主键序列，避免 nextval 与当前最大ID冲突.
     */
    protected function syncPostgresModelSequence($model): void
    {
        if (! $model || DB::getDriverName() !== 'pgsql') {
            return;
        }
    ...
    protected function makeResourceCreator($model)
    {
        $request = request();
        $table = Helper::slug($request->input('table_name'), '_');
        $translateTitle = $request->input('translate_title', '');
    ...
    /**
     * 使用 php -l 验证 PHP 语法，避免使用 eval()
     */
    protected function validatePhpSyntax(string $content, string $backupPath, string $originalPath, string $errorPrefix): void
    {
        ...
        try {
            ...
            if ($exitCode !== 0) {
                ...
                throw new \Exception($errorPrefix . ': ' . implode("\n", $output));
            }
        } finally {
            @unlink($tmpFile);
        }
    ...
    public function addResourceRouteToAdminRoutes($newRoute) {
        ...
        // 验证PHP语法（使用 php -l 代替 eval）
        // $this->validatePhpSyntax($newContent, $backupPath, $routesPath, '生成的路由文件有语法错误');
    ...
    public function addMemberApiResourceRouteToAdminRoutes($newRoute) {
        ...
        // 验证PHP语法（使用 php -l 代替 eval）
        // $this->validatePhpSyntax($newContent, $backupPath, $routesPath, '生成的member路由文件有语法错误');
        ...
        try {
            foreach ($databases as $connectName => $value) {
                ...
                if ($driver === 'mysql' || $driver === 'mariadb') {
                    ...
                    $tables = DB::connection($connectName)->select($tablesSql, $bindings);
                    $databaseName = $value['database'] ?? 'sqlite';
                    $data[$databaseName] = [];
                    foreach ($tables as $tableObj) {
                        $tableName = $tableObj->name;
                        $tableNameWithoutPrefix = Str::replaceFirst($prefix, '', $tableName);
                        // 获取表字段信息（PRAGMA 不支持参数化，tableName 来自数据库查询结果，安全）
                        $columns = DB::connection($connectName)->select('PRAGMA table_info("' . str_replace('"', '""', $tableName) . '")');
                        $columnsArr = [];
                    ...
                    if ($tb) {
                        $tablesSql .= ' AND TABLE_NAME = ?';
                        $tableBindings[] = $prefix . $tb;
                    }

                    $tables = DB::connection($connectName)->select($tablesSql, $tableBindings);
                    ...
                    foreach ($tables as $tableObj) {
                        ...
                        $columns = DB::connection($connectName)->select($columnsSql, [$databaseName, $tableName]);
